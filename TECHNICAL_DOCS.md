...
> 📚 *Демо-проект в образовательных и исследовательских целях.*
...
- **Crypto Analyst Agent** — Криптовалютный аналитик с данными Binance в реальном времени
...
- Интеграция с Binance API для крипто-анализа
...
*   `agents/`: Содержит логику для различных агентов.
    ...
    *   `crypto/`: Агент криптовалютного аналитика.
        *   `agent.py`: Главный координатор с параллельным запуском субагентов.
        *   `prompts.py`: Шаблоны отчётов и форматирование.
        *   `subagents/`: Специализированные субагенты.
            *   `price_analyst.py`: Анализ цены и индикаторов.
            *   `trades_analyst.py`: Анализ потока сделок.
            *   `orderbook_analyst.py`: Анализ стакана.
            *   `news_analyst.py`: Рыночный контекст (Fear & Greed, Market Cap).
...
*   **Native Tool Calling**: Поле `native_tools: true/false` указывает, поддерживает ли модель нативные вызовы функций. В UI для Coder Agent отображается индикатор ✅/⚠️ рядом с названием модели. Для динамических моделей OpenRouter это поле определяется автоматически по префиксу ID модели.
