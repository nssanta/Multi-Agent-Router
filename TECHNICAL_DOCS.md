...
#### `backend/core/tool_calling/text_extractor.py`

*   **Улучшения (v1.1.0)**:
    *   Реализован многоуровневый парсинг ответов LLM.
    *   Добавлены `FALLBACK_PATTERNS` для обработки "сломанного" JSON (например, когда LLM использует f-strings с неэкранированными кавычками внутри JSON).
    *   Специальный паттерн `write_file_broken_json` позволяет извлекать содержимое файлов даже из синтаксически некорректных ответов LLM.
...
*   **Улучшения (v1.1.0)**:
    *   Реализован парсер `parseMessageContent` для полного скрытия технических деталей (JSON tool calls) из сообщений пользователя.
    *   Добавлен `remark-gfm` для поддержки GitHub Flavored Markdown (таблицы, чекбоксы).
    *   Секция "🧠 Ход мыслей" отображается только при наличии текстовых рассуждений (Thoughts) от LLM.
